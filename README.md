...
AI-ассистированной разработки на **1С-Битрикс: Управление сайтом** (PHP). **Не привязан к конкретному AI** — работает
под разные AI-агенты (Claude Code, Cursor, Copilot, Gemini CLI, Codex, Cline, Windsurf, Aider) через единое ядро и
тонкие адаптеры. Не привязан к конкретной организации — адаптируется под любой проект/редакцию. Принцип: **AI не
...
> Статус: **черновик** (собран 24.07.2026 по ресёрчу для встречи с командой). Наполнение конвенциями — по реальному
> коду через `bitrix-toolkit/bitrix-ci`, а не по памяти.
...
Эталон — **Claude Code** (полный: `CLAUDE.md` + скиллы + хуки + MCP). Через `build.sh`/`adapters/` подключаются
**Codex, Gemini CLI** (фолбэк через `AGENTS.md`) и **Cursor, Copilot, Cline, Windsurf, Aider** (через rulesync).
Общий слой (правила, стабы ядра, проверки, MCP) — один на всех.
...
Самостоятельный продукт по AI-разработке Битрикс, по образцу `claude-1c-toolkit`. Обезличен — никаких внутренних
