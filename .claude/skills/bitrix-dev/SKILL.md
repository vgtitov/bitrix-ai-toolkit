...
- **`references/custom-core.md`** — если в проекте **правлено ядро** (`core_modified = true`): сигнатуры и поведение
  брать по ФАКТИЧЕСКОМУ коду ядра, а не по докам вендора; такие места помечать `[правлено ядро]`.
