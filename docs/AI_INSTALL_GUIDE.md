# Автоустановка агентом — развернуть toolkit одной командой

Гайд для AI-агента (Claude Code, Cursor, Codex, Gemini CLI…): развернуть контур на машине/проекте самому, по команде «разверни bitrix-ai-toolkit», без затыков.
...
считать успешной. Если поверх стоит свой глобальный диспетчер git-хуков
(`core.hooksPath` указывает не в репозиторий) — repo-local hooks обычно
вызываются через него; сверься с его настройкой отдельно.
