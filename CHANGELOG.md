...
### Fixed
- Промпт «Начало работы над задачей» (`docs/CONTEXT_AND_KNOWLEDGE.md`) пропускал два самых дешёвых
  и самых часто игнорируемых источника: поиск похожих закрытых задач в трекере (решение может уже
  существовать) и проверку реестра граблей команды. Оба добавлены явными шагами.

## [0.2.1] — 2026-09-14

### Fixed
- Область применения ошибочно исключала Битрикс24: `core/AGENTS.md` требовала «не Битрикс24», хотя
  реальные проекты на практике встречаются в обеих редакциях — сайт может стоять и на классическом
  БУС, и на «Битрикс24 в коробке» (портал как витрина). Заменено на явное определение редакции по
  проекту (`b_option main.vendor`, состав `b_module`) вместо угадывания по своду правил. Портальные
  модули (crm/intranet/tasks/bizproc/…), тянущиеся редакцией, больше не принимаются за бизнес-логику
  магазина без подтверждения по коду.
- `config/version-stack.example.toml`: добавлено значение `edition = "portal"` и пояснение, когда его
  заполнять.

## [0.2.0] — 2026-09-07

### Added
- Механизм хранения контекста: правило одного места истины и что фиксировать по задаче — дефолтом
  в `core/AGENTS.md`; полностью с шаблонами и промптом сбора контекста — `docs/CONTEXT_AND_KNOWLEDGE.md`;
  конкретные места команды — `config/local/knowledge-map.md` по образцу `config/knowledge-map.example.md`
  (нет файла — работают дефолты).

### Fixed
- `commit-msg`: денлист имён AI-инструментов резал легитимные коммиты — фильтр стал структурным.
...
- `commit-msg` хук резал коммиты по денлисту имён AI-инструментов (`claude|copilot|cursor|...`) и
  даже по голому слову «ai»/«ии» — реальный коммит про тему «Скилл интеграции с M365 Copilot»
  стирался целиком. Переведён на структурный фильтр (трейлер/attribution-глагол, не имя) — как в
  claude-1c-toolkit; бэйр-упоминание инструмента как темы больше не режется.
