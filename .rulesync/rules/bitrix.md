...
Область: **1С-Битрикс: сайты и интернет-магазины на PHP.** Не 1С:Предприятие.

**Редакция — не угадывай, определи по проекту.** Магазин может стоять и на «Управление сайтом» (BUS),
и на «Битрикс24 в коробке» (портальная линейка используется как витрина) — на практике встречаются
обе. Проверка: `select main.vendor from b_option` (`bus` — БУС, `1c_bitrix_portal` — портал) или состав
`b_module` (`crm`/`intranet`/`tasks`/`bizproc`/`socialnetwork` — признак портальной линейки).
Портальные модули тянутся редакцией и часто просто крутят агенты фоном — не считай их частью бизнес-логики
магазина, пока это не подтверждено кодом самого проекта. Правила ниже написаны для магазинной части и
применимы к обеим редакциям; где встретится специфика конкретно БУС или конкретно портала — сверяй
по факту, а не по этому своду.
...
## Контекст: одно место истины, решения фиксируются
Факт живёт в одном месте, везде остальное — ссылка. Что нужно в каждой сессии (версии, слои, гейты) —
в репозитории команды; что правят не разработчики — в базе знаний; что про конкретную задачу —
в самой задаче трекера. Персональные данные, секреты и пересказ кода в репозиторий не кладут.

Работая над задачей, фиксируй в неё то, что переживёт сессию: решение и его причину, **отклонённый
вариант с причиной** (иначе его предложат снова), выясненные факты с источником, что проверено по
коду. Не в конце работы, а как только решение принято.

Конкретные места команды — `config/local/knowledge-map.md`, если он есть: он главнее этих дефолтов.
Механизм и шаблоны — `docs/CONTEXT_AND_KNOWLEDGE.md`.
